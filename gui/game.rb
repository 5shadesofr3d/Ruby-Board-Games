...
require_relative 'states/game_states_classic'
require_relative 'debug'
require_relative 'game/view'
require_relative 'game/model'
require_relative 'game/server'
require_relative 'game/client'
...
class Game < Qt::Widget
  ...
  def initialize(rows: 7, columns: 8, width: 800, height: 600, parent: nil)
    ...
    @players = []
    @machine = GameStateMachineClassic.new(self)
  ...
  def setupBoard(rows, cols)
    @board = Board::Board.new(rows, cols, parent: self)
  ...
  def updatePlayers()
    ...
    @players.each {|e| assert e.is_a? Player::Player}
  ...
  def valid?()
    return false unless @board == nil or @board.is_a?(Board::Board)
